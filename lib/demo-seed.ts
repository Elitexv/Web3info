...
/**
 * Populates a fresh (schema-only) database with demo Web3-news content:
 * one admin user, categories, articles with tags, a couple of comments and
 * reactions, and a few newsletter subscribers. Idempotent via upserts, so
 * it's safe to call against a database that already has some of this data.
 */
export async function seedDemoContent(prisma: PrismaClient) {
  ...
  for (const e of SUBSCRIBER_EMAILS) {
  ...
  return { adminEmail: email, adminPassword: password, articleCount: createdArticles.length };
}
